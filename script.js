...
function scrollReveal() {
    const reveals = document.querySelectorAll('.skill-card, .project-card, .contact-card, .fact');
...
revealStyle.textContent = `
    .skill-card, .project-card, .contact-card, .fact {
        opacity: 0;
        transform: translateY(30px);
        transition: all 0.6s ease;
    }
    
    .skill-card.revealed, .project-card.revealed, .contact-card.revealed, .fact.revealed {
        opacity: 1;
...
function animateCounters() {
    ...
    counters.forEach(counter => {
        const target = parseInt(counter.getAttribute('data-width'));
...
if (skillsSection) {
    skillsAnimationObserver.observe(skillsSection);
}
